...
/*
   GFONTDEVICE define here is an application specifc optimized type
   to identify the specific external device where the virtual font
   buffer is located.

   A GFONTDEVICE type parameter is incorporated in each virtual font
   structure as device_id and which are passed as a parameter in the
   getvmem(..) call.
   ...
   A SGUCHAR type is used as default to get a minimum font memory footprint if the
   vf_device parameter is not used by the getvmem(..) function implementation
*/
typedef SGUCHAR GFONTDEVICE;
/*typedef SGULONG GFONTDEVICE;*/ /* ex processor chip select address */
...
/* Called before first call of getvmem
